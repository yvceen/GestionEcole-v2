    ...
    <section class="portal-filter-bar mt-6">
        <form method="GET" data-loading-label="Filtrage des presences..." class="portal-filter-grid lg:grid-cols-[minmax(0,1.2fr)_180px_180px_180px_auto]">
            ...
            <div class="portal-filter-actions">
                ...
                @if($childId || $dateFrom || $dateTo || $status)
                    <a href="{{ route('parent.attendance.index') }}" class="app-button-secondary rounded-2xl px-4 py-3">
                        Reinitialiser
                    </a>
                @endif
            </div>
            ...
            <table class="min-w-full text-sm">
                ...
                <tbody class="divide-y divide-slate-100">
                    @forelse($attendances as $attendance)
                        ...
                        @endphp
                        <tr>
                            <td class="px-3 py-4 font-semibold text-slate-950">{{ $attendance->student?->full_name ?? '-' }}</td>
                            <td class="px-3 py-4 text-slate-600">{{ $attendance->date?->format('d/m/Y') ?? '-' }}</td>
                            <td class="px-3 py-4"><span class="rounded-full px-3 py-1 text-xs font-semibold {{ $tone }}">{{ strtoupper((string) $attendance->status) }}</span></td>
            ...
            @forelse($attendances as $attendance)
                ...
                <article class="portal-record-card">
                    <div class="portal-record-card-head">
                        <div class="min-w-0">
                            <p class="portal-record-title">{{ $attendance->student?->full_name ?? '-' }}</p>
                            <p class="portal-record-subtitle">{{ $attendance->date?->format('d/m/Y') ?? '-' }}</p>
                        </div>
