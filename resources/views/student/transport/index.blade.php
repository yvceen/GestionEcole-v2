<x-student-layout title="Mon transport" subtitle="Consultez votre affectation transport et les derniers pointages montée/descente.">
    <section class="grid gap-4 lg:grid-cols-[minmax(0,1.2fr)_minmax(0,1fr)]">
        <x-ui.card title="Affectation active" subtitle="Route, vehicule, chauffeur et arrêts associés a votre compte.">
            ...
            @if($assignment && $assignment->is_active)
                <div class="space-y-3">
                    ...
                    @if($assignment->pickup_point)
                        <div>
                            <p class="text-sm text-slate-500">Point montée</p>
        ...
        <x-ui.card title="Derniers pointages" subtitle="Historique récent des passages transport.">
            <div class="space-y-2">
                @forelse($logs as $log)
                ...
                @empty
                    <div class="student-empty">Aucun pointage transport enregistré.</div>
                @endforelse
            </div>
        </x-ui.card>
    </section>
