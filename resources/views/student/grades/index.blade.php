    ...
    <section class="grid gap-4 lg:grid-cols-[minmax(0,1.2fr)_320px]">
            ...
            <section class="portal-filter-bar">
                <form method="GET" data-loading-label="Filtrage des notes..." class="portal-filter-grid md:grid-cols-[minmax(0,1fr)_auto]">
                    ...
                    <div class="portal-filter-actions">
                        ...
                        @if($subjectId)
                            <a href="{{ route('student.grades.index') }}" class="app-button-secondary rounded-2xl px-4 py-3">
                                Reinitialiser
                            </a>
                        @endif
                    </div>
                ...
                <div class="hidden overflow-x-auto md:block">
                    <table class="min-w-full text-sm">
                        ...
                        <tbody class="divide-y divide-slate-100">
                            @forelse($gradeRows as $grade)
                                @php
                                    $percent = $grade->max_score ? round(((float) $grade->score / (int) $grade->max_score) * 100, 2) : 0;
                                @endphp
                                <tr>
                                    ...
                                    <td class="px-3 py-4">
                                        <span class="rounded-full bg-slate-100 px-3 py-1 text-xs font-semibold text-slate-700">{{ number_format($percent, 2) }}%</span>
                                    </td>
                                </tr>
                                @if(!empty($grade->comment))
                                    <tr>
                                        <td colspan="5" class="px-3 pb-4 text-xs text-slate-500">Commentaire : {{ $grade->comment }}</td>
                                    </tr>
                                @endif
                            @empty
                                <tr>
                                    <td colspan="5" class="px-3 py-10 text-center text-sm text-slate-500">Aucune note disponible.</td>
                                </tr>
                            @endforelse
                        </tbody>
                    </table>
        ...
        <aside class="space-y-4">
            <section class="student-panel">
                ...
                <p class="mt-2 text-sm text-slate-600">Calculee sur les notes visibles dans votre espace.</p>
            </section>

            <section class="student-panel">
                <p class="text-sm font-semibold text-slate-950">Moyennes par matiere</p>
                <div class="mt-4 space-y-3">
                    @forelse($subjectAverages as $item)
                        <div class="rounded-2xl border border-slate-200 bg-slate-50 px-4 py-3">
                            <div class="flex items-center justify-between gap-3">
                                <p class="font-semibold text-slate-950">{{ $item['subject'] }}</p>
                                <span class="text-sm font-semibold text-slate-950">{{ number_format($item['average_percent'], 2) }}%</span>
                            </div>
                            <p class="mt-1 text-xs text-slate-500">{{ $item['count'] }} note(s) | moyenne brute {{ number_format($item['average_score'], 2) }}</p>
                        </div>
                    @empty
                        <div class="rounded-2xl border border-dashed border-slate-300 bg-slate-50 px-4 py-8 text-sm text-slate-500">
                            Aucune moyenne a afficher.
                        </div>
                    @endforelse
                </div>
            </section>
