...
<x-dynamic-component :component="$layoutComponent" :title="$portalTitle ?? 'Detail du devoir'">
    ...
    >
        <x-slot name="actions">
            ...
            <x-ui.button :href="route($routePrefix . '.edit', $homework)" variant="ghost">
                Modifier
            </x-ui.button>
            ...
            <form method="POST" action="{{ route($routePrefix . '.destroy', $homework) }}">
                @csrf
                @method('DELETE')
                <x-ui.button type="submit" variant="danger" onclick="return confirm('Supprimer ce devoir ?');">
                    Supprimer
                </x-ui.button>
            </form>
        </x-slot>
    ...
    <div class="grid gap-6 lg:grid-cols-[minmax(0,1.4fr)_320px]">
        <x-ui.card>
            ...
            <dl class="mt-5 grid gap-4 text-sm text-slate-700 sm:grid-cols-2">
                <div>
                    <dt class="text-xs uppercase tracking-wide text-slate-500">Enseignant</dt>
                    <dd class="mt-1 font-medium">{{ $homework->teacher?->name ?? '-' }}</dd>
                </div>
                <div>
                    <dt class="text-xs uppercase tracking-wide text-slate-500">Echeance</dt>
                ...
                <div>
                    <dt class="text-xs uppercase tracking-wide text-slate-500">Approuve le</dt>
                    <dd class="mt-1 font-medium">{{ optional($homework->approved_at)->format('Y-m-d H:i') ?? '-' }}</dd>
                </div>
                <div>
                    <dt class="text-xs uppercase tracking-wide text-slate-500">Rejete le</dt>
                    <dd class="mt-1 font-medium">{{ optional($homework->rejected_at)->format('Y-m-d H:i') ?? '-' }}</dd>
                </div>
            </dl>

            @if($homework->description)
                <div class="mt-6 whitespace-pre-line rounded-xl border border-slate-200 bg-slate-50 p-4 text-sm text-slate-700">
                ...
                <ul class="space-y-2 text-sm">
                    @foreach($homework->attachments as $attachment)
                        <li class="flex items-center justify-between gap-3 rounded-lg border border-slate-200 px-3 py-2 text-slate-700">
                            ...
                            <x-ui.button :href="route($routePrefix . '.attachments.download', $attachment)" variant="ghost" size="sm" target="_blank">
                                Telecharger
                            </x-ui.button>
                        </li>
                    @endforeach
