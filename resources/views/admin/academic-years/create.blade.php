<x-admin-layout title="Nouvelle année scolaire" subtitle="Preparez la prochaine année sans toucher a l historique.">
    <x-ui.page-header title="Nouvelle année scolaire" subtitle="Ajoutez une année et choisissez si elle doit devenir l'année courante.">
        <x-slot name="actions">
            <x-ui.button :href="route('admin.academic-years.index')" variant="secondary">Retour aux années</x-ui.button>
        </x-slot>
    </x-ui.page-header>

    <x-ui.card title="Formulaire" subtitle="Les données existantes seront conservees.">
        <form method="POST" action="{{ route('admin.academic-years.store') }}" class="grid gap-4 md:grid-cols-2">
            ...
            <div>
                <label class="mb-2 block text-sm font-medium text-slate-700" for="starts_at">Début</label>
            ...
            <label class="flex items-center gap-3 rounded-2xl border border-slate-200 bg-slate-50 px-4 py-3 text-sm font-medium text-slate-700">
                ...
                Définir cette année comme année courante
            </label>
