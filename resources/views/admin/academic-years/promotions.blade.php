<x-admin-layout title="Passage a l'année suivante" subtitle="Creez les placements de la prochaine année sans dupliquer les comptes.">
    <x-ui.page-header title="Passage a l'année suivante" subtitle="Choisissez l'année source, l'année cible et la nouvelle affectation de chaque Élève.">
        <x-slot name="actions">
            <x-ui.button :href="route('admin.academic-years.index')" variant="secondary">Voir les années</x-ui.button>
        </x-slot>
    </x-ui.page-header>

    <x-ui.card title="Sélection des années" subtitle="L année source conserve son historique. L année cible recoit les nouveaux placements.">
    ...
    @if($targetYear)
        <x-ui.card title="Placements" subtitle="Definissez le statut et la classe cible de chaque Élève.">
            <form method="POST" action="{{ route('admin.academic-promotions.store') }}" class="space-y-6">
                ...
                @forelse($placements as $classroomId => $rows)
                    ...
                    <div class="overflow-hidden rounded-2xl border border-slate-200">
                        <div class="border-b border-slate-200 bg-slate-50 px-5 py-4">
                            ...
                            <p class="mt-1 text-xs text-slate-500">{{ $rows->count() }} Élève(s) a preparer pour {{ $targetYear->name }}</p>
                        </div>
                ...
                @empty
                    <div class="rounded-2xl border border-dashed border-slate-300 bg-white px-6 py-10 text-center text-sm text-slate-500">
                        Aucun placement source disponible pour cette année.
                    </div>
                @endforelse
