<x-admin-layout title="Niveaux">
    <div class="flex items-start justify-between gap-4">
        <div>
            <h1 class="text-2xl font-semibold tracking-tight text-slate-900">Modifier niveau</h1>
            <p class="mt-1 text-sm text-slate-500">{{ $level->name }}</p>
        </div>

        <a href="{{ route('admin.levels.index') }}" class="text-sm font-semibold text-slate-700 hover:underline">← Retour</a>
    </div>
    ...
    <form method="POST" action="{{ route('admin.levels.update', $level) }}"
          class="mt-8 rounded-[28px] border border-white/60 bg-white/70 backdrop-blur-2xl p-6 space-y-4">
        @csrf
        @method('PUT')
        ...
        <div class="grid grid-cols-1 gap-3 sm:grid-cols-2">
            <div>
                ...
                <input name="sort_order" type="number" class="w-full rounded-2xl border-slate-200/70 bg-white/70"
