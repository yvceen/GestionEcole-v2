...
<x-dynamic-component :component="$layoutComponent" title="{{ $event ? 'Modifier Événement' : 'Nouvel Événement' }}">
    <section class="mx-auto max-w-5xl overflow-hidden rounded-[32px] border border-slate-200 bg-white shadow-sm">
        <div class="border-b border-slate-100 bg-[radial-gradient(circle_at_top_right,_rgba(14,165,233,0.16),_transparent_34%),linear-gradient(135deg,#f8fafc,#ffffff)] px-6 py-6">
            <p class="app-overline">Paiements Événements</p>
            <h1 class="mt-2 text-3xl font-semibold tracking-tight text-slate-950">{{ $event ? 'Modifier Événement' : 'Nouvel Événement payant' }}</h1>
            <p class="mt-2 text-sm text-slate-500">Choisissez le montant et les Élèves concernes.</p>
        </div>
        ...
        <form method="POST" action="{{ $action }}" class="space-y-6 px-6 py-6">
            ...
            <div class="grid gap-5 md:grid-cols-2">
                <x-ui.input name="title" label="Nom Événement" :value="old('title', $event?->title)" placeholder="Fête de fin d'année" />
                <x-ui.input name="amount_per_student" label="Montant par Élève (MAD)" type="number" step="0.01" min="0" :value="old('amount_per_student', $event?->amount_per_student)" />
                <x-ui.input name="event_date" label="Date Événement" type="date" :value="old('event_date', optional($event?->event_date)->format('Y-m-d'))" />
            ...
            <div class="grid gap-5 md:grid-cols-2">
                ...
                <div>
                    ...
                    <select id="target_scope" name="target_scope" class="app-input">
                        ...
                        <option value="students" @selected($scope === 'students')>Élèves précis</option>
                    </select>
                </div>
            </div>
            ...
            <div id="students_block" class="hidden rounded-2xl border border-emerald-100 bg-emerald-50/60 px-5 py-5">
                <p class="text-sm font-semibold text-slate-950">Élèves concernes</p>
                    ...
                    @foreach($students as $student)
                        <label class="flex items-center gap-3 rounded-2xl px-4 py-3 text-sm transition hover:bg-slate-50">
                            ...
                            <span class="min-w-0">
                                ...
                                <span class="block text-xs text-slate-500">{{ $student->classroom?->name ?? '-' }} | {{ $student->parentUser?->name ?? 'Parent non lié' }}</span>
                            </span>
                        </label>
                    @endforeach
                </div>
