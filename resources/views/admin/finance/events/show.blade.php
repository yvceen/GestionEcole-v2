...
<x-dynamic-component :component="$layoutComponent" title="Evenement {{ $event->title }}">
    ...
    <section class="mt-6 grid gap-4 md:grid-cols-2 xl:grid-cols-4">
        <div class="rounded-[26px] border border-sky-100 bg-sky-50 px-5 py-5 shadow-sm">
            ...
            <p class="mt-2 text-sm text-slate-500">{{ $targetCount }} eleve(s) concerne(s)</p>
        </div>
        ...
        <div class="rounded-[26px] border border-amber-100 bg-amber-50 px-5 py-5 shadow-sm">
            ...
            <p class="mt-2 text-sm text-slate-500">{{ $partialCount }} partiel(s), {{ $unpaidCount }} non paye(s)</p>
        </div>
    ...
    <section class="mt-5 rounded-[28px] border border-slate-200 bg-white px-5 py-5 shadow-sm">
        ...
        <form method="GET" action="{{ route($routePrefix . '.show', $event) }}" class="grid gap-3 lg:grid-cols-[minmax(0,1.4fr)_220px_220px_auto] lg:items-end">
            <div>
                ...
                <input name="q" value="{{ $q }}" class="app-input" placeholder="Eleve, parent, telephone ou email">
            </div>
            <div>
    ...
    <section class="mt-6 grid gap-6 xl:grid-cols-[minmax(0,1fr)_380px]">
        <div class="rounded-[30px] border border-slate-200 bg-white shadow-sm">
            <div class="border-b border-slate-100 px-6 py-5">
                <h2 class="text-2xl font-semibold tracking-tight text-slate-950">Eleves et paiements</h2>
                <p class="mt-1 text-sm text-slate-500">Selectionnez le parent ou l'eleve, encaissez, puis imprimez le recu.</p>
            </div>
                        ...
                        <div class="grid gap-4 lg:grid-cols-[minmax(0,1fr)_230px]">
                            <div>
                                <div class="flex flex-wrap items-center gap-2">
                                    ...
                                    @else
                                        <x-ui.badge variant="danger">Non paye</x-ui.badge>
                                    @endif
                                    <span class="text-xs font-semibold uppercase tracking-[0.16em] text-slate-400">{{ $target->student?->classroom?->name ?? 'Classe non definie' }}</span>
                                </div>

                                <h3 class="mt-3 text-lg font-semibold text-slate-950">{{ $target->student?->full_name ?? 'Eleve supprime' }}</h3>
                                <p class="mt-1 text-sm text-slate-500">
                                    Parent : <span class="font-semibold text-slate-700">{{ $target->student?->parentUser?->name ?? 'Non lie' }}</span>
                            ...
                            <form method="POST" action="{{ route($routePrefix . '.payments.store', $event) }}" class="rounded-3xl border border-slate-200 bg-slate-50 px-4 py-4">
                                ...
                                @else
                                    <div class="space-y-3">
                                        ...
                                        <div>
                                            <label class="mb-1 block text-xs font-bold uppercase tracking-[0.16em] text-slate-500">Methode</label>
        ...
        <aside class="space-y-5">
            <div class="rounded-[30px] border border-slate-200 bg-white px-5 py-5 shadow-sm">
                <h2 class="text-xl font-semibold text-slate-950">Derniers recus</h2>
                <p class="mt-1 text-sm text-slate-500">Acces rapide aux paiements recents.</p>
                        ...
                        <div class="rounded-2xl border border-slate-200 bg-slate-50 px-4 py-3">
                            <div class="flex items-start justify-between gap-3">
                                <div>
                                    ...
                                    <p class="mt-1 text-xs text-slate-500">{{ $payment->parent?->name ?? 'Parent non lie' }} | {{ $payment->paid_at?->format('d/m/Y H:i') }}</p>
                                </div>
                            ...
                            <div class="mt-3 flex items-center justify-between gap-3">
                                ...
                                <a href="{{ route($routePrefix . '.payments.receipt', $payment) }}" class="text-xs font-bold text-sky-700 hover:text-sky-900">Recu</a>
                            </div>
                        </div>
                    @empty
            ...
            <div class="rounded-[30px] border border-rose-100 bg-rose-50 px-5 py-5">
                ...
                <p class="mt-1 text-sm text-rose-700">Supprimez seulement si l'evenement est une erreur. Les paiements lies seront aussi retires.</p>
                <form method="POST" action="{{ route($routePrefix . '.destroy', $event) }}" class="mt-4" onsubmit="return confirm('Supprimer cet evenement et ses paiements ?')">
                    @csrf
                    @method('DELETE')
                    <x-ui.button type="submit" variant="danger" class="w-full justify-center">Supprimer l'evenement</x-ui.button>
                </form>
            </div>
        </aside>
    </section>
