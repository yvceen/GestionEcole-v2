...
<x-dynamic-component :component="$layoutComponent" title="Parametres emploi du temps">
    <x-ui.page-header
        title="Parametres emploi du temps et présences"
        subtitle="Definissez la journée type, les seances, les heures souples et les réglés de retard/absence."
    />
    ...
    <x-ui.card title="Reglages generaux" subtitle="Ces parametres servent de base a l agenda, aux creneaux et aux réglés de pointage.">
        <form method="POST" action="{{ route($routePrefix . '.settings.update') }}" class="space-y-6">
            @csrf
            @method('PUT')

            <div class="grid gap-4 md:grid-cols-2">
                <x-ui.input id="day_start_time" type="time" name="day_start_time" label="Heure de début" :value="old('day_start_time', substr((string) $settings->day_start_time, 0, 5))" required />
                ...
                <x-ui.input id="late_grace_minutes" type="number" min="0" max="120" step="1" name="late_grace_minutes" label="Delai de grace retard (minutes)" :value="old('late_grace_minutes', (int) ($settings->late_grace_minutes ?? 15))" required hint="Ex: 15 = retard a partir de 08:15 si début a 08:00." />
                <x-ui.input id="auto_absent_cutoff_time" type="time" name="auto_absent_cutoff_time" label="Heure limite auto absent (optionnel)" :value="old('auto_absent_cutoff_time', $settings->auto_absent_cutoff_time ? substr((string) $settings->auto_absent_cutoff_time, 0, 5) : '')" hint="Si vide, la derniere fin de session ou 09:00 sera utilisée." />
                ...
                <div class="rounded-2xl border border-slate-200 bg-slate-50 px-4 py-4 text-sm text-slate-600">
                    ...
                    <p class="mt-1 text-xs leading-5">Vous pouvez garder une plage globale large puis définir des sessions de présence plus fines juste en dessous.</p>
                </div>
                <x-ui.input id="lunch_start" type="time" name="lunch_start" label="Pause dejeuner - début" :value="old('lunch_start', $settings->lunch_start ? substr((string) $settings->lunch_start, 0, 5) : '')" />
            ...
            <div class="rounded-3xl border border-slate-200 bg-slate-50 px-5 py-5">
                <div class="flex items-center justify-between gap-3">
                    <div>
                        <h3 class="text-sm font-semibold text-slate-900">Sessions de présence horaire</h3>
                        <p class="mt-1 text-xs text-slate-500">Definissez plusieurs plages dans la journée si l'École fonctionne en seances distinctes.</p>
                    </div>
                ...
                <div class="mt-4 space-y-3">
                    @foreach($sessions as $index => $session)
                        <div class="grid gap-3 md:grid-cols-3">
                            ...
                            <x-ui.input :id="'attendance-session-start-'.$index" type="time" :name="'attendance_sessions['.$index.'][start]'" label="Début" :value="data_get($session, 'start')" />
