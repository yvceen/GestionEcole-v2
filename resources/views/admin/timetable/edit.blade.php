...
<x-dynamic-component :component="$layoutComponent" title="Modifier l'emploi du temps">
    ...
    <x-ui.card title="Creneau" subtitle="Mettez a jour les horaires, la matiere, l'enseignant et la salle.">
        <form method="POST" action="{{ route($routePrefix . '.update', $timetable) }}" class="space-y-6">
            @csrf
            @method('PUT')
            ...
            <div class="flex items-center gap-3 border-t border-slate-200 pt-4">
                ...
                <x-ui.button :href="route($routePrefix . '.index', ['classroom_id' => $timetable->classroom_id])" variant="secondary">Retour</x-ui.button>
            </div>
        </form>
    </x-ui.card>
</x-dynamic-component>
