{{-- resources/views/admin/school-life/edit.blade.php --}}
<x-admin-layout title="Modifier entree">
    <x-slot name="header">Modifier entree</x-slot>

    <div class="max-w-3xl mx-auto px-4 sm:px-6 py-6">
        <div class="flex items-center justify-between">
            <h1 class="text-2xl font-semibold text-slate-900">Modifier entree</h1>
        ...
        <form method="POST" action="{{ route('admin.school-life.update', $schoolLife) }}" class="mt-6 space-y-4 rounded-[28px] border border-black/10 bg-white/80 backdrop-blur-xl p-6 shadow-sm">
            @csrf
            @method('PUT')

            <div>
                <label class="block text-xs font-semibold text-slate-600 mb-1">Titre</label>
                <input name="title" value="{{ old('title', $schoolLife->title) }}" class="w-full rounded-2xl border border-black/10 bg-white px-4 py-2.5 text-sm">
            ...
            <div>
                ...
                <select name="status" class="w-full rounded-2xl border border-black/10 bg-white px-4 py-2.5 text-sm">
                    @foreach(['draft' => 'Brouillon', 'published' => 'Publie', 'archived' => 'Archive'] as $val => $label)
                        <option value="{{ $val }}" @selected(old('status', $schoolLife->status) === $val)>{{ $label }}</option>
                    @endforeach
                </select>
            ...
            <div>
                ...
                <input type="date" name="date" value="{{ old('date', optional($schoolLife->date)->format('Y-m-d')) }}" class="w-full rounded-2xl border border-black/10 bg-white px-4 py-2.5 text-sm">
