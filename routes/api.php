...
use App\Http\Controllers\Api\MobileDigitalAuthorizationController;
use App\Http\Controllers\Api\MobileDocumentRequestController;
use App\Http\Controllers\Api\MobileBehaviorFeedController;
use App\Http\Controllers\Api\MobileFinanceController;
use App\Http\Controllers\Api\MobileFeedbackCaseController;
use App\Http\Controllers\Api\MobileGradesController;
use App\Http\Controllers\Api\MobileHomeworkController;
use App\Http\Controllers\Api\MobileHealthController;
...
Route::middleware([IdentifySchoolFromSubdomain::class])->group(function (): void {
    ...
    Route::middleware([AuthenticateMobileApiToken::class, SetCurrentSchool::class, EnsureApiSchoolAccess::class])->group(function (): void {
        ...
        Route::get('/health', [MobileHealthController::class, 'index'])->name('api.health.index');
        Route::post('/health/reports', [MobileHealthController::class, 'store'])->name('api.health.reports.store');
        Route::get('/digital-authorizations', [MobileDigitalAuthorizationController::class, 'index'])->name('api.digital-authorizations.index');
        Route::post('/digital-authorizations/{recipient}/respond', [MobileDigitalAuthorizationController::class, 'respond'])->name('api.digital-authorizations.respond');
        Route::get('/document-requests', [MobileDocumentRequestController::class, 'index'])->name('api.document-requests.index');
        Route::post('/document-requests', [MobileDocumentRequestController::class, 'store'])->name('api.document-requests.store');
        Route::post('/document-requests/{documentRequest}/cancel', [MobileDocumentRequestController::class, 'cancel'])->name('api.document-requests.cancel');
        Route::get('/feedback', [MobileFeedbackCaseController::class, 'index'])->name('api.feedback.index');
        Route::post('/feedback', [MobileFeedbackCaseController::class, 'store'])->name('api.feedback.store');
        Route::get('/feedback/{feedbackCase}', [MobileFeedbackCaseController::class, 'show'])->name('api.feedback.show');
        Route::post('/feedback/{feedbackCase}/reply', [MobileFeedbackCaseController::class, 'reply'])->name('api.feedback.reply');
